#!/usr/bin/env bb
;; torifune 鳥船 — launch-vehicle-ledger persistence + heartbeat tests.
;; Run:  bb --classpath 20-actors 20-actors/torifune/tests/test_kotoba.cljc
(ns torifune.tests.test-kotoba
  (:require [torifune.methods.kotoba :as k]
            [torifune.methods.autorun :as auto]
            [kotoba.lang.text :as str]
            [clojure.test :refer [deftest is run-tests]]
            [clojure.java.io :as io]))

(defn- tmp [] (str (System/getProperty "java.io.tmpdir") "/torifune-ledger-test-" (gensym) ".edn"))
(defn- d1 [] [(k/add "vehicle:ama" ":organism/kind" ":vehicle")
              (k/add "vehicle:ama" ":vehicle/reusable" true)])
(defn- d2 [] [(k/add "en.stage:s1.contributes.vehicle:ama" ":en/kind" ":contributes")
              (k/add "en.stage:s1.contributes.vehicle:ama" ":en/dv-contribution" 5400.0)])
...
(deftest float-dv-contribution-roundtrips
  (let [p (tmp)]
    (try
      (k/append-tx (k/make-tx (d2) "t1" "as1" "") p)
      (is (= (d2) (get (first (k/read-log p)) ":tx/datoms")) "float :en/dv-contribution round-trips")
      (finally (io/delete-file p true)))))
  ...
  (let [p (tmp)]
    (try
      (let [c1 (k/append-tx (k/make-tx (d1) "t1" "as1" "") p)]
        ...
        (let [corrupted (str/replace (slurp p) ":contributes" ":strikes")]
          (spit p corrupted)
...
;; ── heartbeat (autorun) over the REAL committed Ama-vehicle seed ─────────────

(deftest beat-ground-only-and-real-seed
  (let [ds (auto/ground-datoms)]
    (is (seq ds) "ground datoms are produced from the real committed Ama-vehicle seed")
    (is (every? #(= ":db/add" (first %)) ds) "every datom is an :add (EAVT op)")
    (is (some (fn [[_ _ a]] (= ":en/from" a)) ds) "launch-vehicle 縁 are persisted (graph not just nodes)")
    (is (not-any? (fn [[_ _ a]] (str/starts-with? (str a) ":bond/")) ds)
        "GROUND only — derived :bond/* readouts not persisted (N1)")))

(deftest beat-no-weapon-delivery
  ;; G1: a CIVILIAN launch vehicle — no strike/munition/MIRV/FOBS/ballistic datoms
  (let [ds (auto/ground-datoms)]
    (is (not-any? (fn [d] (some #(let [s (str %)]
                                   (or (str/includes? s "strike") (str/includes? s "munition")
                                       (str/includes? s "mirv") (str/includes? s "fobs")
                                       (str/includes? s "warhead"))) d)) ds)
        "no strike/munition/MIRV/FOBS/warhead in the ledger (G1, civilian-launch-only)")))
...
#?(:clj
   (when (= *file* (System/getProperty "babashka.file"))
     (let [{:keys [fail error]} (run-tests 'torifune.tests.test-kotoba)]
