(ns torifune.methods.social
  "social.cljc — torifune DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from torifune's own
...
(defn draft-observation-post
  "A dry-run observation post over torifune's own domain (Zero-net-carbon open launch-vehicle manufacturing + Transparent space access — the 船大工 of the sky (funadaiku/sarutahiko/giemon…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "torifune R0: live social posting is Council Lv6+ + operator + "
