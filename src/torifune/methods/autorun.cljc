#!/usr/bin/env bb
;; torifune 鳥船 — autonomous heartbeat: load the launch-vehicle graph → append GROUND datoms.
(ns torifune.methods.autorun
  "autorun.cljc — torifune 鳥船 deterministic heartbeat (ADR-2606162355, subaru/inochi pattern).

  One beat: load the Ama-class launch-vehicle seed graph, build the GROUND EAVT datoms (vehicle/
  stage/engine/propellant/plant-cell/mission/disposal NODES + their :en/* 縁), and APPEND them
  as ONE content-addressed transaction to the append-only LAUNCH-VEHICLE LEDGER (kotoba.cljc).
  prev-cid chaining keeps the ledger tamper-evident + resume-safe.

  GROUND ONLY: ascent Δv margin / carbon balance / deorbit debt are DERIVED (computed on read,
  N1) and are NEVER persisted. Deterministic by construction: the caller supplies tx-id + as-of
  (no wall clock); nodes emit in EDN-read order (ascent-sim/node-ids) + edges content-stable →
...
#?(:clj
   (defn default-seed-path []
     (str (io/file here-dir "data" "seed-ama-vehicle.kotoba.edn"))))
...
(defn beat
  "Run one heartbeat. opts:
     ...
     :log-path launch-vehicle-ledger path (required)
...
#?(:clj
   (defn -main [& args]
     (let [log-path (or (first args)
                        (str (io/file here-dir "data" "persisted" "torifune.launch-vehicle.kotoba.edn")))
           r (beat {:tx-id "torifune-beat-manual" :as-of "manual" :log-path log-path})]
       (println (str "launch-vehicle ledger head=" (:head r)
                     " datoms=" (:count r)
                     " appended=" (:appended r)
