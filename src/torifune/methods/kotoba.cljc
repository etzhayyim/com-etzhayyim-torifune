#!/usr/bin/env bb
;; torifune 鳥船 — launch-vehicle graph DATOM-LOG writer (content-addressed commit-DAG).
(ns torifune.methods.kotoba
  "kotoba.cljc — torifune 鳥船 append-only LAUNCH-VEHICLE LEDGER writer
  (ADR-2606162355, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the subaru/inochi/tate/rasen/ugachi/busshi family — the
  datomic-isomorphic canonical-state layer torifune previously only EMITTED (datom_emit)
  but never PERSISTED).

  Persists torifune's GROUND datoms — the vehicle / stage / engine / propellant / plant-cell /
  mission / disposal NODES and their :en/* 縁 (PUBLIC open-design facts, G3) — to a local
  append-only EDN log of content-addressed transactions. DERIVED state (ascent Δv margin /
  carbon balance / deorbit debt, computed on READ, N1) is NEVER stored.
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; torifune 鳥船 — LAUNCH-VEHICLE LEDGER (append-only, content-addressed EAVT "
                        "commit-DAG of the vehicle/stage/engine/propellant/mission/disposal graph). "
                        "Generated; DO NOT hand-edit. CIVILIAN launch only — strike/munition trajectories "
                        "structurally unrepresentable. ADR-2606162355 / ADR-2605312345.\n")))
